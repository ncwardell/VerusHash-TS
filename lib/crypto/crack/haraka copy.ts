 ...
 */
function hexbyte (x: { toString: (arg0: number) => any }) {
 ...
 * Print state
 */
function printstate (s: string | any[]) {
  let q
 ...
 */
function aesenc (s: any[], rk: string | any[]) {
  s = subbytes(s)
  s = shiftrows(s)
  s = mixcolumns(s)
...
export function aes2(s1: Uint8Array, s2: Uint8Array, r: number, rc: Uint8Array): void {
  // Example (assuming aesenc takes a Uint8Array as the round key):
  const roundKey1 = xor(Array.from(rc.subarray(r * 16, (r + 1) * 16)), Array.from(s1));
  const roundKey2 = xor(Array.from(rc.subarray((r + 1) * 16, (r + 2) * 16)), Array.from(s2));

  s1.set(aesenc(Array.from(s1), roundKey1));
  s2.set(aesenc(Array.from(s2), roundKey2));
}

export function mix2(s1: Uint8Array, s2: Uint8Array): void {
  // Implement MIX2 logic here
  const tmp1 = new Uint8Array(16);
  const tmp2 = new Uint8Array(16);
  tmp1.set(s1.subarray(0, 8));
  tmp1.set(s2.subarray(0, 8), 8);
  tmp2.set(s2.subarray(8, 16));
  tmp2.set(s1.subarray(8, 16), 8);
 ...
 */
/**
 * Keyed Haraka-512/256
 */
export function haraka512256Keyed(msg: number[], key: Uint8Array): number[] {
  ...
  for (let t = 0; t < ROUNDS; ++t) {
    ...
    for (let m = 0; m < AES_ROUNDS; ++m) {
      for (let i = 0; i < 4; ++i) {
        // Integrate the key into the round constants (example)
        const roundConstant = convRC(RC[4 * t * AES_ROUNDS + 4 * m + i]);
        for (let j = 0; j < roundConstant.length; ++j) {
          roundConstant[j] ^= key[i * 16 + j];
        }
        s[i] = aesenc(s[i], roundConstant);
      }
    }
 ...
 * Haraka-256/256
 */
export function haraka256256 (msg: number[]) {
  // obtain state from msg input
  let s = [
    [
      /* eslint-disable computed-property-spacing */
...
function main () {
  ...
  // call Haraka-512/256
  let digest = haraka512256Keyed(m, new Uint8Array(64))
